package com.example.mediapipeimageclassification
...
class ImageClassifierHelper(
    ...
    val modelName: String = "mobilenet-v1-tflite-1-0-224-quantized-metadata-v1.tflite",
    ...
    private fun setupImageClassifier() {
        ...
        if (runningMode == RunningMode.LIVE_STREAM) {
            optionsBuilder.setResultListener { result, _ ->
                ...
                val interfaceTime = finishTimeMs - result.timestampMs()
                classifierListener?.onResults(
                    result.classificationResult().classifications(), interfaceTime
                )
            }.setErrorListener { error -> classifierListener?.onError(error.message.toString()) }
        }
