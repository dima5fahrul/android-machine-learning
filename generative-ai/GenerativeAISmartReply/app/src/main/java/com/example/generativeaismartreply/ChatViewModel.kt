...
class ChatViewModel : ViewModel() {
    ...
    fun addMessage(message: String) {
        ...
        val list: ArrayList<Message> = _chatHistory.value ?: ArrayList()
    ...
    private fun initSmartReplyOptionsGenerator() {
        _smartReplyOptions.addSource(pretendingAsAnotherUser) { isPrentendingAsAnotherUser ->
            val list = chatHistory.value

            if (list.isNullOrEmpty()) {
                return@addSource
            } else {
                generateSmartReplyOptions(list, isPrentendingAsAnotherUser)
    ...
    private fun generateSmartReplyOptions(
        ...
        val chatConversation = ArrayList<TextMessage>()
        ...
        return smartReply.suggestReplies(chatConversation)
            .continueWith { task ->
